...
# This code calculates and graphs days below freezing at AZMET stations where
...
for (s in 1:length(stn_names)) {
  ...
  if (s == 1) {
    stn_data_daily <- s_data
  } else {
    stn_data_daily <- rbind(stn_data_daily, s_data)
  }
}
rm(stn, s_data)
rm(s)
...
for (s in 1:length(stn_names)) {
  ...
  x <- filter(stn_data_daily, Station == stn, Year == yr_start)
...
stn_data_daily["below28"] <- NA
stn_data_daily["bet2832"] <- NA

measurements <- which(is.na(stn_data_daily$Tmin) == FALSE)

stn_data_daily$below28[measurements] <- 0
stn_data_daily$bet2832[measurements] <- 0

rm(measurements)

below28 <- which(stn_data_daily$Tmin < 28)

stn_data_daily$below28[below28] <- 1

rm(below28)

bet2832 <- which(stn_data_daily$Tmin >= 28 & stn_data_daily$Tmin < 32)

stn_data_daily$bet2832[bet2832] <- 1
...
ggplot() +
  ...
  geom_line(data = stn_data_monthly,
            mapping = aes(x = Year, y = N_Days, color = Station),
            size = 2) +
